...
Goldsky subgraphs and pipelines that index onchain events for Herald and forward them to the Herald webhook. Each standard (ERC-8004, ERC-8183, ...) gets its own subgraph and pipeline under `subgraphs/` and `pipelines/`.
...
1. Add `subgraphs/<standard>/<version>/` with the ABI and subgraph manifest, following `subgraphs/identity-registry/1.0.0` as the template.
