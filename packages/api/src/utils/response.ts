...
export const unexpectedError = <TContext extends Context>(c: TContext) =>
  c.json<HttpErrorResponse<typeof INTERNAL_SERVER_ERROR>>(
    { code: INTERNAL_SERVER_ERROR, message: "Unexpected error occurred" },
    500
  );

export function ok<TContext extends Context>(
  c: TContext
): Response & TypedResponse<JSONParsed<undefined>, 200, "json">;
