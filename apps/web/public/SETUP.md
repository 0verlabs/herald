...
The skills in [`skills/`](https://github.com/spaceobject-ai/skills/tree/main/skills) are the source for how the platform works. This step makes them loadable in your tool.

- **Claude Plugin**: this repo is a plugin marketplace —
  ```
...
- **Claude Code, OpenCode, Codex, Pi, etc.**:
  ...
  ```
- **Clone repo**: copy the skill folders into whatever directory your tool reads `SKILL.md` skills from, e.g.:
  ```bash
  cp -r skills/spaceobject skills/spaceobject-client ~/.claude/skills/
  ```
  Discovered locations vary by tool (`.opencode/skills/`, `~/.config/opencode/skills/`, `.claude/skills/`, `.agents/skills/`); OpenCode can instead add the clone's `skills/` directory to any `opencode.jsonc`:
  ```jsonc
  { "skills": ["<path-to-clone>/skills"] }
  ```
  If your tool has no skill mechanism, read the `SKILL.md` files directly — start with [`skills/spaceobject/SKILL.md`](https://github.com/spaceobject-ai/skills/blob/main/skills/spaceobject/SKILL.md), then your role's file.
